...
def count_biggest_return_winners():
    ...
    if not graphing_mode:
        if len(actual_winners) != 0:
            change_label(f'Players {actual_winners} win.')
            time.sleep(2)
            
        if len(tiers) != 0:
            change_label(f'Players {tiers} tie.')
            time.sleep(2)
...
def go_do_it():
    ...
    while True:
        ...
        for i in range(1, number_of_players + 1):
            ...
            #print_cards(i)
            refresh_list_box()
            change_label(f'Player {i}\'s turn')
            
            if player_and_dealer_list[i].npc_controller:
                ask_for_decision(i)
            else:
        ...
        if len(busted) != number_of_players:
            ...
            while True:
                ...
                if player_and_dealer_list[0].cards_values > 21:
                    ...
                    if not graphing_mode:
                        change_label(f'Players {winners} win.')
                        time.sleep(1)
                    break
...
def give_hint():
    if waiting_for_user:
        change_label(f'Percentage of getting less than 21\n{calculate_win_percentage(player_and_dealer_list[current_user].cards_values)}%')

# holds cards
def hold_card():
    global waiting_for_user
    if waiting_for_user:
        #print_cards(current_user)
        change_label('Hold')
        refresh_list_box()
...
def hit_card():
    ...
    if waiting_for_user:
        ...
        if player_and_dealer_list[current_user].cards_values > 21:
            ...
            change_label('Hit, Pick Again')

# refreshes cards in listbox
def refresh_list_box():
    if not graphing_mode:
        player_card_box.delete(0, END)
        
        for players in player_and_dealer_list:
            if player_and_dealer_list.index(players) != 0:
                player_card_box.insert(END, f'Player {player_and_dealer_list.index(players)} cards: {players.cards_list} {players.cards_values}')
            else:
                player_card_box.insert(END, f'Dealer cards: {players.cards_list} {players.cards_values}')
...
player_card_box = Listbox(root, width=30)
player_card_box.pack(padx=20, pady=20)
