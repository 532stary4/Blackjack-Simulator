...
def create_playing_deck(number, playing_deck):
    for j in range(1,14):
        playing_deck[j] = (number * 4)

def shuffle_new_deck(leftover_pile, playing_deck, left_in_deck):
    #print('')
    #print('Shuffle')
    #print('')
...
def deal_card_to_player(player_number, playing_deck, left_in_deck, player_and_dealer_list, leftover_pile):
    if len(left_in_deck) == 0:
        shuffle_new_deck(leftover_pile, playing_deck, left_in_deck)
...
def create_player(actual_players, player_and_dealer_list):
    if actual_players != 0:
...
def start_game(number_of_players, playing_deck, left_in_deck, player_and_dealer_list, leftover_pile):
    for players in range(number_of_players + 1):
        deal_card_to_player(players, playing_deck, left_in_deck, player_and_dealer_list, leftover_pile)
...
def ask_for_decision(player_number, busted, player_and_dealer_list, playing_deck, left_in_deck, leftover_pile):
    while True:
        if player_and_dealer_list[player_number].cpu_controller:
            decision = calculate_next_move(player_number, player_and_dealer_list, playing_deck, leftover_pile)
        else:
            #print(f'Player {player_number} cards: {player_and_dealer_list[player_number].cards_list} {player_and_dealer_list[player_number].cards_values}')
            decision = int(input(f'Player {player_number}, hold (0), hit (1) or hint (2)? '))
        
        if decision == 1:
            deal_card_to_player(player_number, playing_deck, left_in_deck, player_and_dealer_list, leftover_pile)
...
def calculate_win_percentage(current_value, playing_deck, leftover_pile):
    return round(((count_up_to((21 - current_value), playing_deck) / count_cards_in_deck(playing_deck, leftover_pile)) * 100), 2)

def calculate_next_move(player_number, player_and_dealer_list, playing_deck, leftover_pile):
    ...
    if strength == 1:
        ...
        percentage = calculate_win_percentage(player_and_dealer_list[player_number].cards_values, playing_deck, leftover_pile)
...
def count_biggest_return_winners(player_and_dealer_list, busted):
    temp = {}
...
def count_cards_in_deck(playing_deck, leftover_pile):
    output = 0
    ...
    if output == 0:
        shuffle_new_deck(leftover_pile, playing_deck)
        output = count_cards_in_deck()
    
    return output

def count_up_to(in_num, playing_deck):
    output = 0
...
def make_graph(player_and_dealer_list):
    chart = pygal.Line()
...
def begin_the_program():
    ...
    create_playing_deck(number_of_decks, playing_deck)
    ...
    for players in range(number_of_players):
        create_player(actual_players, player_and_dealer_list)
    ...
    while True:
        ...
        start_game(number_of_players, playing_deck, left_in_deck, player_and_dealer_list, leftover_pile)
        ...
        for i in range(1, number_of_players + 1):
            #print('')
            print_cards(i)
            ask_for_decision(i, busted, player_and_dealer_list, playing_deck, left_in_deck, leftover_pile)
        
        #print('')
        
        if len(busted) != number_of_players:
            while True:
                deal_card_to_player(0, playing_deck, left_in_deck, player_and_dealer_list, leftover_pile)
                print_cards(0)
                ...
                if player_and_dealer_list[0].cards_values > 21:
                    ...
                    count_biggest_return_winners(player_and_dealer_list, busted)
                    break
        ...
        if times == 0:
            ...
            #make_graph(player_and_dealer_list)
            break
